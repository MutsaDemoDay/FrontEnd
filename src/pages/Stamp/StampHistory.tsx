...
const StampHistory = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setIsLoading(true);
        const data = await fetchStampHistory();

        // API 데이터를 화면용 데이터 구조로 변환
        const formattedData: HistoryItem[] = data.map(
          (item: StampHistoryResponse, index: number) => ({
            id: index, // API에 고유 ID가 없다면 인덱스 사용
            date: item.issuedDate.replace(/-/g, '.'), // '2025-11-08' -> '2025.11.08' 변환
            cafeName: item.storeName,
            address: item.storeAddress,
          })
        );

        // 최신 날짜가 위로 오게 하려면 reverse() 등을 사용 가능.
        // 현재는 API 순서 그대로 사용
        setHistoryData(formattedData);
      } catch (error) {
        console.error('데이터 로드 실패:', error);
        // 에러 처리 로직 (예: 토스트 메시지 등)
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // --- [드래그 이벤트 핸들러] ---
  const handleStart = (clientX: number) => {
    if (isLoading || historyData.length === 0) return; // 데이터 없으면 드래그 방지
    setIsDragging(true);
    startX.current = clientX;
  };
  ...
  if (historyData.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white">
        <BackButton />
        <p className="text-gray-500 mt-4">스탬프 적립 내역이 없습니다.</p>
      </div>
    );
  }
      ...
      >
        ...
        <div className="relative w-[160px] h-[250px] mb-12">
          {[-4, -3, -2, -1, 0, 1, 2, 3, 4].map((offsetStep) => {
            ...
            // --- API 연결 시 데이터 가져오는 로직 ---
            // 헬퍼 함수를 사용해 안전하게 가져옵니다.
            // historyData가 비어있으면 위에서 이미 로딩/빈화면 처리를 했으므로 여기선 안전합니다.
            // 다만 렌더링 로직상 map 안에서 직접 getItem 호출은 필요 없습니다 (시각적 효과만 계산).

            // 실제 카드 안에 내용을 뿌려주거나 key가 필요하다면 아래처럼 가져옵니다.
            // 현재 코드는 시각적인 카드 스택이므로 key 생성이 중요합니다.
            const stackItem = getItem(renderIndex);
